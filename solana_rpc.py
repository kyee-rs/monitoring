...
def load_identity_account_pubkey(config: ValidatorConfig) -> Optional[str]:
    ...
    """
    identity_cmd = f'solana address -u localhost --keypair ' + config.secrets_path + '/identity.json'
...
def load_vote_account_pubkey(config: ValidatorConfig) -> Optional[str]:
    ...
    """
    vote_pubkey_cmd = f'solana address -u localhost --keypair ' + config.secrets_path + '/vote.json'
